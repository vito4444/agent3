...
- M2 面板:T = 科技树(选定研究目标后,把数据核搬到研究台由研究员消耗);J = 职业配额;建造模式下 [ ] 翻页选择已解锁建筑(数字键选前 9 项)。锁定建筑不出现在菜单,蓝图命令对锁定项一律拒绝。
- 速度:空格暂停;F1/F2/F3 = 1×/2×/4×;危急警报默认自动暂停(设置 `settings.json` 的 `AutoPauseOnCritical` 可关)。
- 存读档:F5 存到 `persistentDataPath/save_v1.json.gz`(路径见日志);F9 读档。F10 开发者叠层。
